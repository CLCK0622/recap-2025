...
import {TbAppWindow} from "react-icons/tb"; // 用作"小窗口"图标
import {FaArrowRight} from "react-icons/fa";

// ==========================================
// WebGL Shader & Class Definitions (保持不变)
// ==========================================
...
class Geometry {
    ...
    public get data(): { vertices: Float32Array; indices: Uint16Array; normals: Float32Array; uvs: Float32Array; } {
        return {
            ...
            images.forEach((img, i) => {
                if (img.width > 0 && img.height > 0) {
                    ...
                    if (imgAspect > cellAspect) {
                        // 图片比格子宽：高度撑满，宽度按比例缩放（裁剪左右）
                        drawHeight = cellSize;
                        drawWidth = cellSize * imgAspect;
                        offsetY = 0;
                        offsetX = (cellSize - drawWidth) / 2; // 水平居中
                    } else {
                        // 图片比格子高：宽度撑满，高度按比例缩放（裁剪上下）
                        drawWidth = cellSize;
                        drawHeight = cellSize / imgAspect;
                        offsetX = 0;
                        offsetY = (cellSize - drawHeight) / 2; // 垂直居中
                    }

                    ctx.save();
                    // 1. 创建裁剪区域，防止图片画出当前格子的范围
                    ctx.beginPath();
                    ctx.rect(x, y, cellSize, cellSize);
                    ctx.clip();

                    // 2. 绘制图片（居中并覆盖）
                    // drawImage 参数: img, x, y, width, height
                    // 这里的 x/y 需要加上格子在 Atlas 中的基础坐标 (x, y) 以及居中偏移量
...
// ==========================================
// React Component (Modified UI Logic)
// ==========================================
